...
class CustomBundle[T <: Data](elts: (String, T)*)(implicit compileOptions: CompileOptions) extends Record {
...
class CustomIndexedBundle[T <: Data](elts: (Int, T)*)(implicit compileOptions: CompileOptions) extends Record {
  // Must be String, Data
  val elements = ListMap(elts map { case (field, elt) => field.toString -> elt.cloneType }: _*)

  // TODO: Make an equivalent to the below work publicly (or only on subclasses?)
  def indexedElements = ListMap(elts map { case (field, elt) => field -> elt.cloneType }: _*)
