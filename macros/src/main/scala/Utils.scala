...
object Utils {
  ...
  def readConfFromString(str: String): Seq[mdf.macrolib.Macro] = {
    MemConf.fromString(str).map { m:MemConf =>
      SRAMMacro(m.name, m.width, m.depth, Utils.portSpecToFamily(m.ports), "", 1, Utils.portSpecToMacroPort(m.width, m.depth, m.maskGranularity, m.ports), Seq.empty[MacroExtraPort])
    }
  }
  ...
  def buildSRAMMacros(s: mdf.macrolib.SRAMCompiler): Seq[mdf.macrolib.SRAMMacro] = {
    for (g <- s.groups; d <- g.depth; w <- g.width; vt <- g.vt)
      yield mdf.macrolib.SRAMMacro(makeName(g, d, w, vt), w, d, g.family, vt, g.mux, g.ports.map(_.copy(width=Some(w), depth=Some(d))), g.extraPorts)
  }
  def buildSRAMMacro(g: mdf.macrolib.SRAMGroup, d: Int, w: Int, vt: String): mdf.macrolib.SRAMMacro = {
    return mdf.macrolib.SRAMMacro(makeName(g, d, w, vt), w, d, g.family, vt, g.mux, g.ports.map(_.copy(width=Some(w), depth=Some(d))), g.extraPorts)
  }
