...
import chisel3.stage.{ChiselGeneratorAnnotation, ChiselCircuitAnnotation, ChiselStage}
...
trait ReparentCompilerTest { this: TestSuite =>
  protected def annos: AnnotationSeq = AnnotationSeq(Seq(ReParentCircuitAnnotation(ModuleTarget("DummyTestHarness", "RocketTile"))))
...
class ReparentSpec extends AnyFreeSpec with ReparentCompilerTest {
  ...
  "thing" in {
    val (firrtl, annos) = compile(new DummyTestHarness, "high", a = Seq(RunFirrtlTransformAnnotation(new ReParentCircuit)))
    println(firrtl)
    annos.toSeq.foreach(println)
    val tileTopAnnos = annos.filter( _ match {
      case EmittedFirrtlCircuitAnnotation(efc) =>
        efc match {
          case EmittedFirrtlCircuit(name, circuit, sfc) => 
            name == "RocketTile"
          case _ => false
        }
      case _ => false
